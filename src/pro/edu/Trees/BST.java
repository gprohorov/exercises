...
public class BST {
    ...
    public boolean deleteNode(int value){
        ...
        if (currentNode.getLeftChild() == null && currentNode.getRightChild() == null) { // узел просто удаляется, если не имеет потомков
            ...
            Node heir = getHeir(currentNode);// поиск преемника для удаляемого узла
            if (currentNode == rootNode)
                rootNode = heir;
            else if (isLeftChild)
    ...
    public void printTree() { // метод для вывода дерева в консоль
      ...
      public Set<Node> getAllChilds(Node node){
        Set<Node> set = new HashSet<>();
          if(node ==null) return  set;
          Node currentNode = node;

        while(node.getRightChild()!= null || node.getLeftChild()!= null){
            Set<Node> right = getAllRightChilds(node);
            Set<Node> left = getAllLeftChilds(node);
            set.addAll(right);
            set.addAll(left);
            set = getAllLeftChilds(node.getRightChild());
            set = getAllChilds(node.getLeftChild());
        }
        return set;
      }
      private Set<Node> getAllRightChilds(Node node){
          Set<Node> set = new HashSet<>();
          if(node ==null) return  set;
          Node currentNode = node;
          while( currentNode.getRightChild() != null){
              set.add(currentNode.getRightChild());
              currentNode = currentNode.getRightChild();
          }
          return set;
      }

      private Set<Node> getAllLeftChilds(Node node){
          Set<Node> set = new HashSet<>();
          if(node ==null) return  set;
          Node currentNode = node;
          while( currentNode.getLeftChild() != null){
              set.add(currentNode.getLeftChild());
              currentNode = currentNode.getLeftChild();
          }
          return set;
      }

      public int getAmountOfChilds(Node node){
        return getAllChilds(node).size();
      }
